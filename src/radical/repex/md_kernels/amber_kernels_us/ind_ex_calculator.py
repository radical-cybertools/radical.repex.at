"""
.. module:: radical.repex.md_kernels.amber_kernels_us.ind_ex_calculator
...
if __name__ == '__main__':
    ...
    replica_cycle = str(data["replica_cycle"])
    ...
    while (success == 0):
        try:
            replica_energy, path_to_replica_folder = get_historical_data(replica_path=replica_path, history_name=history_name)
                ...
                try:
                    ...
                    with open(outfile, 'w+') as f:
                        ...
                        for item in swap_column:
                            ...
                            f.write(row_str)
                            f.write('\n')
                            row_str = replica_id + " " + str(replica_cycle) + " " + new_restraints + " " + str(init_temp)
                            f.write(row_str)
                        f.close()
    ...
    for j in range(replicas):  
        success = 0        
        current_rstr = all_restraints[j]

        new_coor = "%s_%d_%s.rst" % (base_name, j, replica_cycle)
        replica_path = "../staging_area/replica_%d/" % (j)
        r_coor = replica_path + new_coor
        while (success == 0):
            try:
                #---------------------------------------------------------------
